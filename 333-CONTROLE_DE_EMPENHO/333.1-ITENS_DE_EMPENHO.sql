SELECT DISTINCT
    ...
    CASE 
        WHEN :vis_lote = 'S' THEN COALESCE(TOTAIS.TOTALEMP, 0)
        WHEN :vis_lote = 'N' THEN (
            SELECT SUM(COALESCE(TGFITE.QTDNEG, 0))
            FROM TGFITE
            INNER JOIN TGFCAB ON TGFITE.NUNOTA = TGFCAB.NUNOTA
            WHERE TGFITE.SEQUENCIA > 0
            AND TGFCAB.STATUSNOTA = 'L'
            AND TGFCAB.CODTIPOPER IN ('1951','1950')
            AND TGFITE.CODPROD = TPRLMP.CODPRODMP
            AND TGFCAB.IDIPROC = TPRIPROC.IDIPROC
        )
    END AS TOTALEMP,
    CASE 
        WHEN :vis_lote = 'S' THEN TOTAIS.CONTROLE
        WHEN :vis_lote = 'N' THEN NULL
    END AS LOTE,
